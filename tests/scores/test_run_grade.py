...
def test_headwind_uses_pace_to_increase_relative_air_speed():
    faster = grade(average_pace_minutes_per_km=4, headwind_kph=20)
    slower = grade(average_pace_minutes_per_km=6, headwind_kph=20)
...
def test_tailwind_cannot_make_relative_air_speed_negative():
    result = grade(average_pace_minutes_per_km=10, headwind_kph=-20)
...
@pytest.mark.parametrize(
    ("overrides", "message"),
    [
        ({"average_pace_minutes_per_km": 0}, "greater than zero"),
        ({"average_pace_minutes_per_km": float("nan")}, "pace must be finite"),
